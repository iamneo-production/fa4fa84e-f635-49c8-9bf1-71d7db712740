...
export class AdminappliedloanComponent implements OnInit {
  ...
  constructor(private fb:FormBuilder  ,private adminService: AdminService, private http: HttpClient, private router:Router ,private notif:ToastrService) {
  ...
  handleReject(loanId: number) {
    console.log(loanId)
      this.adminService.deleteLoan(loanId).subscribe((res: any)=>{
        console.log(res);
        this.notif.success("Deleted","Successfully",{timeOut:1000 })
        this.fetchAppliedLoans();
      }); 
  }
  ...
  this.adminService.approveMultipleLoans(selectedIds.map(Number)).subscribe(
    (res: any) => {
      if (res.Status === 'Success') {
        ...
        this.notif.success('Approved','Successfully',{timeOut:1000})
        this.ngOnInit();
      } else {
        console.log(res);
      }
  ...
  if (applicant.selected) {
    ...
    adminDeleteLoan(id:number){
      ...
      this.adminService.deleteLoan(id).subscribe((res: any)=>{
        console.log(res);
        this.notif.success('Success',"Application Deleted",{timeOut :1000});
        this.fetchAppliedLoans();
      }); 
    }
